...
function Users() {
	...
	const handleFormModal = async () => {
		...
		if(formData.id !== ''){
			await axios.post(baseURLAPI('/admin/users/update/'+formData.id),formData,{withCredentials:true})
			...
			await axios.post(baseURLAPI('/admin/users/insert'),formData,{withCredentials:true})
			.then(function (response) {
				setDraw(draw + 1);
				handleModal(false)
				setIsSubmitForm(false);
	...
	const handleDeleteForm    = async (e,id) =>{
		...
		alertConfirm("Apakah kamu yakin akan menghapus data ini", async () => {
			await axios.post(baseURLAPI('/admin/users/delete/'+id),formData,{withCredentials:true})
			.then(function (response) {
				setDraw(draw + 1);
				alertSuccess(response.data.message)
			})
	...
	return (
		<>
			<PanelNavbar page={"users"}>
				...
				<Datatable draw={draw} config={config} url={'/admin/users/all'}/>
								...
								<div className="p-6 space-y-6">
									...
									<div>
										<div className="mb-6">
											...
											<input name='username' onChange={handleChange} value={formData.username} type="text" id="username" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Input Username"/>
										</div> 
										<div className="mb-6">
											<label htmlFor="password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Password</label>
											<input name='password' onChange={handleChange} value={formData.password} type="password" id="password" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="•••••••••"/>
										</div> 
										<div className="mb-6">
											<label htmlFor="role" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Role</label>
											<select name='role' defaultValue={''} onChange={handleChange} id="role" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
												<option value="">Pilih</option>
												<option selected={formData.role === "kasir"} value="kasir">Kasir</option>
												<option selected={formData.role === "admin"} value="admin">Admin</option>
											</select>
										</div>
									</div>
								</div>
