...
function Menu() {
	...
	if(role === "admin"){
		...
		body 	= [
			...
			},
			{
				render : (data) => {
					return (
						<img src={data.url_gambar} className='w-64'/>
					)
				}
			},
			{
	...
	const handleChange = e => {
		...
		if(name === "gambar"){
			value 	= files.length > 0 ? files[0] : [];
			formData.url_gambar = URL.createObjectURL(value) ? URL.createObjectURL(value) : "";
	...
	const handleFormModal = async () => {
		...
		if(formData.id !== ''){
			await axios.post(baseURLAPI('/admin/menu/update/'+formData.id),formData,{
			...
			await axios.post(baseURLAPI('/admin/menu/insert'),formData,{
				withCredentials:true,
				headers: {
					"Content-Type": "multipart/form-data",
				},
	...
	const handleEditForm    = (e,data) =>{
		...
		setFormData({
			...
			harga_menu : data.harga_menu.replace(/[^0-9]/g,''),
	...
	const handleDeleteForm    = async (e,id) =>{
		...
		alertConfirm("Apakah kamu yakin akan menghapus data ini", async () => {
			await axios.post(baseURLAPI('/admin/menu/delete/'+id),formData,{withCredentials:true})
			.then(function (response) {
				setDraw(draw + 1);
				alertSuccess(response.data.message)
			})
	...
	const handleGetKategoriMenu = async () => {
		await axios.get(baseURLAPI('/admin/kategori_menu/all'),{withCredentials:true})
		.then(function (response) {
			setKategoriMenu(response.data.data);
		});
	}

	useEffect(() => {
		handleGetKategoriMenu();	
	},[]);
	...
	return (
		<>
			<PanelNavbar page={"menu"}>
				...
				<Datatable draw={draw} config={config} url={'/admin/menu/all'}/>
								...
								<form id='formSubmit' onSubmit={handleFormModal}>
									<div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
										...
										<div className="p-6 space-y-6">
											...
											<div>
												<div className='mb-6'>
													<img src={formData.url_gambar} className='max-w-96 max-h-48 rounded-lg mb-4 mx-auto'/>
													<input name='gambar' id='gambar' accept='.jpg,.jpeg,.png,.bmp,.webp' onChange={handleChange} className="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" type="file"></input>
												</div>
												<div className="mb-6">
													<label htmlFor="nama_menu" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Menu</label>
													<input name='nama_menu' onChange={handleChange} value={formData.nama_menu} type="text" id="nama_menu" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Input Nama Menu"/>
												</div>
												<div className="mb-6">
													<label htmlFor="harga_menu" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Harga Menu</label>
													<input name='harga_menu' onChange={handleChange} value={formData.harga_menu} type="text" id="harga_menu" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Input Harga Menu"/>
												</div>
												<div className="mb-6">
													<label htmlFor="deskripsi_menu" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Deskripsi Menu</label>
													<textarea name='deskripsi_menu' value={formData.deskripsi_menu} rows={5} onChange={handleChange} id="deskripsi_menu" className="block resize-none p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Input Deskripsi Menu"></textarea>
												</div>
												<div className="mb-6">
													<label htmlFor="id_kategori_menu" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kategori Menu</label>
													<select onChange={handleChange} name='id_kategori_menu' id="id_kategori_menu" value={formData.id_kategori_menu} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
														<option value="">Pilih Kategori Menu</option>
														{
															kategoriMenu.map((kategori) => (
																<option key={kategori.id} value={kategori.id}>{kategori.nama_kategori_menu}</option>
															))
														}
													</select>
												</div>
